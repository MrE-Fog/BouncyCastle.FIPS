...
namespace Org.BouncyCastle.Cert
{
		...
		// TODO Add interface Crl?
	{
        ...
        public X509Crl(byte[] encoding): this(CertificateList.GetInstance(encoding))
        {
        }
			...
			CertificateList c)
		{
			...
			try
			{
				this.sigAlgName = SignatureUtilities.GetAlgorithmName(c.SignatureAlgorithm);
            ...
            IVerifierFactory<AlgorithmIdentifier> verifier)
        {
            ...
            {
                throw new InvalidSignatureException("CRL does not verify with supplied public key");
            }
        }
		...
		public override string ToString()
		{
			...
			if (extensions != null)
			{
				...
				do
				{
					...
					if (ext.Value != null)
					{
						Asn1Object asn1Value = Asn1Object.FromByteArray(ext.Value.GetOctets());
...
//				BigInteger serial = ((X509Certificate)cert).SerialNumber;
		...
		protected virtual bool IsIndirectCrl
		{
			get
			{
				byte[] idp = GetExtensionValue(X509Extensions.IssuingDistributionPoint);
