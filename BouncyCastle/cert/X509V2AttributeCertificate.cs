...
namespace Org.BouncyCastle.Cert
{
		...
		: X509ExtensionBase
	{
            ...
            IVerifierFactory<AlgorithmIdentifier> verifier)
        {
			...
			{
				throw new CertificateException("Exception encoding certificate info object", e);
			}

			if (!(streamCalculator.GetResult()).IsVerified(this.GetSignature()))
			{
				throw new CertificateException("Public key presented not for certificate signature");
			}
		}
