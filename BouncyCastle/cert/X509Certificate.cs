...
namespace Org.BouncyCastle.Cert
{
    ...
    public class X509Certificate: X509ExtensionBase
    {
        ...
        public X509Certificate(byte[] certData): this(X509CertificateStructure.GetInstance(certData))
        {
        }
			...
			X509CertificateStructure c)
		{
			...
			try
			{
				byte[] str = this.GetExtensionValue(X509Extensions.BasicConstraints);

				if (str != null)
				{
                    basicConstraints = BasicConstraints.GetInstance(str);
				}
			}
			catch (Exception e)
			{
			...
			try
			{
                byte[] str = this.GetExtensionValue(new DerObjectIdentifier("2.5.29.15"));

				if (str != null)
				{
                    DerBitString bits = DerBitString.GetInstance(str);
		...
		public virtual X509CertificateStructure ToAsn1Structure()
		{
			 return c;
		}
...
//        public IPrincipal IssuerDN
		public virtual X500Name IssuerDN
		{
            get { return c.Issuer; }
        }
...
//        public IPrincipal SubjectDN
		public virtual X500Name SubjectDN
		{
		...
		{
            get { return SignatureUtilities.GetAlgorithmName(c.SignatureAlgorithm); }
		}
		...
		public virtual DerObjectIdentifier SigAlgOid
		{
            get { return c.SignatureAlgorithm.Algorithm; }
		}
		...
		{
			byte[] str = this.GetExtensionValue(new DerObjectIdentifier("2.5.29.37"));
			...
			try
			{
				Asn1Sequence seq = Asn1Sequence.GetInstance(str);
		...
		protected virtual ICollection GetAlternativeNames(
			string oid)
		{
			byte[] altNames = GetExtensionValue(new DerObjectIdentifier(oid));

			if (altNames == null)
				return null;

			Asn1Object asn1Object = Asn1Object.FromByteArray(altNames);
...
//			return Arrays.AreEqual(this.GetEncoded(), other.GetEncoded());
			...
			IVerifierFactory<AlgorithmIdentifier> verifier)
		{
			...
			{
				throw new InvalidSignatureException("certificate does not verify with supplied public key");
			}
		}
