...
namespace Org.BouncyCastle.Crypto.Internal.Pqc.Crypto.Sphincs
{
    class Permute
    {
        private static readonly int CHACHA_ROUNDS = 12;


        protected static int rotl(int x, int y)
        {
            return (int)(((uint)x << y) | ((uint)x >> -y));
        }
        ...
        internal void chacha_permute(byte[] output, byte[] input)
        {
            int i;
            ...
            for (i = 0; i < 16; i++)
            {
                x[i] = (int)Pack.LE_To_UInt32(input, 4 * i);
            }
            ...
            for (i = 0; i < 16; ++i)
            {
                Pack.UInt32_To_LE((uint)x[i], output, 4 * i);
            }
        }
    }
}
