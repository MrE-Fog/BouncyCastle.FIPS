...
namespace Org.BouncyCastle.Crypto.Internal.Engines
{
	/// <summary>
		...
		: IStreamCipher
	{
			...
			ICipherParameters	parameters)
		{
            ...
            KeyParameter keyParam = parameters as KeyParameter;
            if (keyParam != null)
            {
                this.key = keyParam;
                return;
            ...
            ParametersWithRounds roundParams = parameters as ParametersWithRounds;
            if (roundParams != null)
            {
                this.rounds = roundParams.Rounds;
                parameters = roundParams.Parameters;  
            }
            ...
            if (ivParams.Parameters != null)
                throw new ArgumentException(AlgorithmName + " Init expects separate key initialization");

            byte[] iv = ivParams.GetIV();
            if (iv == null || !IsValidNonceSize(iv.Length))
                throw new ArgumentException(AlgorithmName + " IV cannot be " + iv.Length + " bytes");

            if (this.key == null)
				throw new ArgumentException(AlgorithmName + " Init requires a key", "parameters");

			SetKey(this.key.GetKey(), iv);
			Reset();
			initialised = true;
		}

        protected virtual bool IsValidNonceSize(int nonceSize)
        {
            return nonceSize == 8;
        }
