...
{
    internal class NewHope
    {
        private const bool StatisticalTest = false;

        internal const int AgreementSize = 32;
        internal const int PolySize = Params.N;
        internal const int SendABytes = Params.PolyBytes + Params.SeedBytes;
        internal const int SendBBytes = Params.PolyBytes + Params.RecBytes;

        internal static void KeyGen(SecureRandom rand, byte[] send, ushort[] sk)
        {
            Debug.Assert(send.Length >= SendABytes);
            Debug.Assert(sk.Length >= PolySize);

            byte[] seed = new byte[Params.SeedBytes];
            rand.NextBytes(seed);

            ushort[] a = new ushort[Params.N];
            GenerateA(a, seed);
            ...
            ushort[] e = new ushort[Params.N];
            Poly.GetNoise(e, noiseSeed, (byte)1);
            Poly.ToNtt(e);

            ushort[] r = new ushort[Params.N];
            Poly.PointWise(a, sk, r);

            ushort[] pk = new ushort[Params.N];
            Poly.Add(r, e, pk);

            EncodeA(send, pk, seed);
        }

        internal static void SharedB(SecureRandom rand, byte[] sharedKey, byte[] send, byte[] received)
        {
            Debug.Assert(sharedKey.Length >= AgreementSize);
            Debug.Assert(send.Length >= SendBBytes);
            Debug.Assert(received.Length >= SendABytes);

            ushort[] pkA = new ushort[Params.N];
            byte[] seed = new byte[Params.SeedBytes];
            ...
            ushort[] sp = new ushort[Params.N];
            Poly.GetNoise(sp, noiseSeed, (byte)0);
            Poly.ToNtt(sp);

            ushort[] ep = new ushort[Params.N];
            Poly.GetNoise(ep, noiseSeed, (byte)1);
            Poly.ToNtt(ep);

            ushort[] bp = new ushort[Params.N];
            Poly.PointWise(a, sp, bp);
            Poly.Add(bp, ep, bp);

            ushort[] v = new ushort[Params.N];
            Poly.PointWise(pkA, sp, v);
            Poly.FromNtt(v);

            ushort[] epp = new ushort[Params.N];
            Poly.GetNoise(epp, noiseSeed, (byte)2);
            Poly.Add(v, epp, v);

            ushort[] c = new ushort[Params.N];
        ...
        internal static void SharedA(byte[] sharedKey, ushort[] sk, byte[] received)
        {
            Debug.Assert(sharedKey.Length >= AgreementSize);
            Debug.Assert(sk.Length >= PolySize);
            Debug.Assert(received.Length >= SendBBytes);
        ...
        private static void DecodeA(ushort[] pk, byte[] seed, byte[] r)
        {
            Debug.Assert(r.Length >= SendABytes);
            Debug.Assert(pk.Length >= PolySize);
            Debug.Assert(seed.Length >= Params.SeedBytes);

            Poly.FromBytes(pk, r);
            Array.Copy(r, Params.PolyBytes, seed, 0, Params.SeedBytes);
        }

        private static void DecodeB(ushort[] b, ushort[] c, byte[] r)
        {
            Poly.FromBytes(b, r);
            ...
            {
                int j = 4 * i;
                uint ri = r[Params.PolyBytes + i];
                c[j + 0] = (ushort)(ri & 0x03U);
                c[j + 1] = (ushort)((ri >> 2) & 0x03U);
                c[j + 2] = (ushort)((ri >> 4) & 0x03U);
                c[j + 3] = (ushort)(ri >> 6);
            }
        }

        private static void EncodeA(byte[] r, ushort[] pk, byte[] seed)
        {
            Debug.Assert(r.Length >= SendABytes);
            Debug.Assert(pk.Length >= PolySize);
            Debug.Assert(seed.Length >= Params.SeedBytes);
            ...
            Poly.ToBytes(r, pk);
            Array.Copy(seed, 0, r, Params.PolyBytes, Params.SeedBytes);
        }

        private static void EncodeB(byte[] r, ushort[] b, ushort[] c)
        {
            Debug.Assert(r.Length >= SendBBytes);
            Debug.Assert(b.Length >= PolySize);
            Debug.Assert(c.Length >= PolySize);
            ...
            {
                int j = 4 * i;
                r[Params.PolyBytes + i] = (byte)(c[j] | (c[j + 1] << 2) | (c[j + 2] << 4) | (c[j + 3] << 6));
            }
        }

        private static void GenerateA(ushort[] a, byte[] seed)
        {
            Poly.Uniform(a, seed);
        }

        private static void Sha3(byte[] sharedKey)
        {
            Sha3Digest d = new Sha3Digest(256);
            d.BlockUpdate(sharedKey, 0, 32);
        ...
        private class ChaCha20
        {
            internal static void Process(byte[] key, byte[] nonce, byte[] buf, int off, int len)
            {
                ...
                e.Init(true, new ParametersWithIV(null, nonce));
                e.ProcessBytes(buf, off, len, buf, off);
            }
        }

        private class ErrorCorrection
        {
            ...
            internal static int F(int[] v, int off0, int off1, int x)
            {
                ...
                return Abs(x - ((v[off0]) * 2 * (int)Params.Q));
            }
            ...
            internal static void HelpRec(ushort[] c, ushort[] v, byte[] seed, byte nonce)
            {
                byte[] iv = new byte[8];
                //iv[7] = nonce;
                iv[0] = nonce;
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    c[0 + i  ] = (ushort)((vTmp[0] -     vTmp[3]) & 3);
                    c[256 + i] = (ushort)((vTmp[1] -     vTmp[3]) & 3);
                    c[512 + i] = (ushort)((vTmp[2] -     vTmp[3]) & 3);
                    c[768 + i] = (ushort)((   -k   + 2 * vTmp[3]) & 3);
                }
            }

            internal static ushort LDDecode(int xi0, int xi1, int xi2, int xi3)
            {
                int t;
                ...
                return (ushort)((uint)t >> 31);
            }

            internal static void Rec(byte[] key, ushort[] v, ushort[] c)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    key[i >> 3] |= (byte)(LDDecode(tmp[0], tmp[1], tmp[2], tmp[3]) << (i & 7));
                }
            }
        }
        ...
        private class Ntt
        {
            private static readonly ushort[] BitReverseTable = { 0, 512, 256, 768, 128, 640, 384, 896, 64, 576, 320, 832, 192, 704,
            ...
            internal static void BitReverse(ushort[] poly)
            {
             ...
             */
            internal static void Core(ushort[] a, ushort[] omega)
            {
                int distance;
                for (int i = 0; i < 10; i += 2)
                {
                    ...
                    for (int start = 0; start < distance; ++start)
                    {
                        ...
                        {
                            uint u = a[j], v = a[j + distance], w = omega[jTwiddle++];
                            a[j] = (ushort)(u + v); // Omit reduction (be lazy)
            ...
            internal static void MulCoefficients(ushort[] poly, ushort[] factors)
            {
                for (int i = 0; i < Params.N; ++i)
                {
                    uint xi = poly[i], yi = factors[i];
        ...
        private class Params
        {
            ...
            internal const uint Q = 12289;

            internal const int PolyBytes = 1792;
            internal const int RecBytes = 256;
            internal const int SeedBytes = 32;
        }

        private class Poly
        {
            internal static void Add(ushort[] x, ushort[] y, ushort[] z)
            {
                for (int i = 0; i < Params.N; ++i)
                {
                    z[i] = Reduce.Barrett((ushort)(x[i] + y[i]));
                }
            }

            internal static void FromBytes(ushort[] r, byte[] a)
            {
                Debug.Assert(r.Length >= PolySize);
                Debug.Assert(a.Length >= Params.PolyBytes);

                for (int i = 0; i < Params.N / 4; ++i)
                {
                    int j = 7 * i;
                    uint a0 = a[j + 0], a1 = a[j + 1], a2 = a[j + 2], a3 = a[j + 3], a4 = a[j + 4], a5 = a[j + 5], a6 = a[j + 6];

                    int k = 4 * i;
                    r[k + 0] = (ushort)( a0                   | ((a1 & 0x3F) <<  8));
                    r[k + 1] = (ushort)((a1 >> 6) | (a2 << 2) | ((a3 & 0x0F) << 10));
                    r[k + 2] = (ushort)((a3 >> 4) | (a4 << 4) | ((a5 & 0x03) << 12));
                    r[k + 3] = (ushort)((a5 >> 2) | (a6 << 6));
                }
            }

            internal static void FromNtt(ushort[] r)
            {
                Ntt.BitReverse(r);
                Ntt.Core(r, Precomp.OmegasInvMontgomery);
                Ntt.MulCoefficients(r, Precomp.PsisInvMontgomery);
            }

            internal static void GetNoise(ushort[] r, byte[] seed, byte nonce)
            {
                Debug.Assert(Params.K == 16);
                Debug.Assert(r.Length >= Params.N);
                Debug.Assert(seed.Length >= Params.SeedBytes);
                ...
                for (int i = 0; i < Params.N; ++i)
                {
                    uint t = BitConverter.ToUInt32(buf, i * 4);
                    //r[i] = (ushort)(BitCount(t) + Params.Q - Params.K);
                    ...
                    uint a = ((d >> 24) + (d >> 0)) & 0xFF;
                    uint b = ((d >> 16) + (d >> 8)) & 0xFF;
                    r[i] = (ushort)(a + Params.Q - b);
                }
            }

            internal static void PointWise(ushort[] x, ushort[] y, ushort[] z)
            {
                for (int i = 0; i < Params.N; ++i)
                {
                    uint xi = x[i], yi = y[i];
                    ushort t = Reduce.Montgomery(3186 * yi);        // t is now in Montgomery domain
                    z[i] = Reduce.Montgomery(xi * (uint)t);         // z[i] is back in normal domain
                }
            }

            internal static void ToBytes(byte[] r, ushort[] p)
            {
                Debug.Assert(r.Length >= Params.PolyBytes);
                Debug.Assert(p.Length >= PolySize);
                ...
                for (int i = 0; i < Params.N / 4; ++i)
                {
                    ...
                    ushort t0 = Normalize(p[j + 0]);
                    ushort t1 = Normalize(p[j + 1]);
                    ushort t2 = Normalize(p[j + 2]);
                    ushort t3 = Normalize(p[j + 3]);
            ...
            internal static void ToNtt(ushort[] r)
            {
                Ntt.MulCoefficients(r, Precomp.PsisBitrevMontgomery);
                Ntt.Core(r, Precomp.OmegasMontgomery);
            }

            internal static void Uniform(ushort[] a, byte[] seed)
            {
                ...
                for (;;)
                {
                    ...
                    {
                        uint val = output[i] | ((uint)output[i + 1] << 8);
                        val &= 0x3FFFU;
                        if (val < Params.Q)
                        {
                            a[pos++] = (ushort)val;
            ...
            //    n = ((n + (n >> 4) & 0x0F0F0F0FU) * 0x01010101U) >> 24;
            //    return (int)n;
            //}

            private static ushort Normalize(ushort x)
            {
                int t = Reduce.Barrett(x);
                int m = t - (int)Params.Q;
                int c = m >> 31;
                t = m ^ ((t ^ m) & c);
                return (ushort)t;
            }
        ...
        private class Precomp
        {
            internal static readonly ushort[] OmegasMontgomery = { 4075, 6974, 7373, 7965, 3262, 5079, 522, 2169, 6364, 1018, 1041, 8775,
            ...
            internal static readonly ushort[] OmegasInvMontgomery = { 4075, 5315, 4324, 4916, 10120, 11767, 7210, 9027, 10316, 6715, 1278,
            ...
            internal static readonly ushort[] PsisBitrevMontgomery = { 4075, 6974, 7373, 7965, 3262, 5079, 522, 2169, 6364, 1018, 1041,
            ...
            internal static readonly ushort[] PsisInvMontgomery = { 256, 10570, 1510, 7238, 1034, 7170, 6291, 7921, 11665, 3422, 4000,
        ...
        {
            private const uint QInv = 12287; // -inverse_mod(p,2^18)
            private const int RLog = 18;
            private const uint RMask = (1 << RLog) - 1;

            internal static ushort Montgomery(uint a)
            {
                uint u = a * QInv;
                u &= RMask;
                u *= Params.Q;
                u += a;
                return (ushort)(u >> RLog);
            }

            internal static ushort Barrett(ushort a)
            {
                uint t = a;
                uint u = (t * 5) >> 16;
                u *= Params.Q;
