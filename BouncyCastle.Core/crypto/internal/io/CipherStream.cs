...
namespace Org.BouncyCastle.Crypto.Internal.IO
{
		...
		: Stream
    {
		...
		internal CipherStream(
			Stream			stream,
			IBufferedCipher	cipher) : this(stream, stream.CanRead ? cipher : null, stream.CanWrite ? cipher : null)
		{
		...
		internal IBufferedCipher ReadCipher
		{
			get { 
		...
		internal IBufferedCipher WriteCipher
		{
			get {
				CryptoServicesRegistrar.ApprovedModeCheck (isApprovedModeOnly, "CipherStream");
