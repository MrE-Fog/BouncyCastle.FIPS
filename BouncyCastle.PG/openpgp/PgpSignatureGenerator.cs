...
namespace Org.BouncyCastle.OpenPgp
{
    ...
    public class PgpSignatureGenerator
    {
        ...
        private PgpSignatureIdentifier signatureId;
        private IStreamCalculator<IBlockResultWithDigest> signer;
            ...
            bool isNested)
        {
            return new PgpOnePassSignature(
				new OnePassSignaturePacket(
					signatureType, signatureId.HashAlgorithm, signatureId.KeyAlgorithm, signatureId.KeyId, isNested));
        }
        ...
        public PgpSignature Generate()
        {
			...
			{
				unhPkts = insertSubpacket(unhPkts, new IssuerKeyId(false, signatureId.KeyId));
			}
			...
			try
            {
                ...
                sOut.WriteByte((byte)signatureId.KeyAlgorithm);
                sOut.WriteByte((byte)signatureId.HashAlgorithm);
			...
			signer.Stream.Write(hData, 0, hData.Length);
            ...
            IBlockResultWithDigest result = signer.GetResult();

			byte[] sigBytes = result.Collect();
            byte[] digest = result.CollectDigest();
			byte[] fingerPrint = new byte[] { digest[0], digest[1] };

			// an RSA signature
			bool isRsa = signatureId.KeyAlgorithm == PublicKeyAlgorithmTag.RsaSign
				|| signatureId.KeyAlgorithm == PublicKeyAlgorithmTag.RsaGeneral;
			...
			return new PgpSignature(
				new SignaturePacket(signatureType, signatureId.KeyId, signatureId.KeyAlgorithm,
                    signatureId.HashAlgorithm, hPkts, unhPkts, fingerPrint, sigValues));
        }
