...
namespace Org.BouncyCastle.OpenPgp
{
    ...
    public class PgpSignature
    {
		...
		private IStreamCalculator<IVerifier>	verifier;
        ...
        {
            IVerifierFactory<PgpSignatureTypeIdentifier> verifierFactory = verifierFactoryProvider.CreateVerifierFactory(new PgpSignatureTypeIdentifier(sigPck.KeyAlgorithm, sigPck.HashAlgorithm));
			lastb = 0;

            verifier = verifierFactory.CreateCalculator();
        }
		...
		{
            verifier.Stream.WriteByte((byte)'\r');
            verifier.Stream.WriteByte((byte)'\n');
		}
            ...
            int		length)
        {
            ...
            {
                verifier.Stream.Write(bytes, off, length);
            }
        }
			...
			PgpPublicKey					key)
		{
			...
			return verifier.GetResult().IsVerified(this.GetSignature());
		}
